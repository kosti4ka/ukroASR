...
if [ $# -ne 3 ]; then
  echo "Usage: prepare_local_dict_from_lm.sh <src-dict-dir> <lm-path> <out-dict-dir>"
...
ngram -lm $lm_path -write-vocab $out_dict_dir/vocab
# remove </s> <s> <unk> from vocab
sed -i '/<*>/d' $out_dict_dir/vocab
