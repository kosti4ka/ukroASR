...
REALIGN_THRESHOLD = 10


def update_alignments(input_audio_path, original_json_path, updated_json_path, out_json_path):
...
# parts.append((current_line, total_lines, current_time, total_len))
    ...
    original_j = json.load(open(original_json_path, 'r', encoding='utf8'))
    updated_j = json.load(open(updated_json_path, 'r', encoding='utf8'))
    ...
    for f_original, f_updated in zip(original_j['fragments'], updated_j['fragments']):
        assert f_original['id'] == f_updated['id']
        utt_id = f_updated['id']
        lines = f_updated['lines']
        if round(float(f_original['begin']), 3) != round(float(f_updated['begin']), 3) or '<<<<<<<<<' in lines:
            begin_fixed = True
            if '<<<<<<<<<' not in lines:
                lines = ['<<<<<<<<<'] + lines
        if round(float(f_original['end']), 3) != round(float(f_updated['end']), 3) or '>>>>>>>>>' in lines:
            end_fixed = True
            if '>>>>>>>>>' not in lines:
                lines = lines + ['>>>>>>>>>']

        out_j['fragments'].append({'id':utt_id,
                                   "children": f_updated['children'],
                                   "language": f_updated['language'],
                                   'begin':float(f_updated['begin']),
                                   'end':float(f_updated['end']),
                                   'lines':lines
               })

        if begin_fixed and not end_fixed:
            curent_start_to_align = round(float(f_updated['begin']),3)
        ...
        if not begin_fixed and end_fixed:
            curent_end_to_align = round(float(f_updated['end']), 3)
            current_text_to_align.append((utt_id, lines[0]))
    ...
    for segment in segments_to_align:
        ...
        ffmpeg_cmd = f'ffmpeg -y -ss {start_time} -i {input_audio_path} -t {duration} {audio_segment}'
    ...
    with open(out_json_path, 'w', encoding='utf8') as fp:
        json.dump(out_j, fp, ensure_ascii=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-oj', '--original_json_path', required=False, type=Path,
                        help='Path to the original json file')
    parser.add_argument('-uj', '--updated_json_path', required=False, type=Path,
                        help='Path to the manually updated json file')
    ...
    if not script_args.original_json_path and script_args.input_text_path.exists():
        run_aligning_task(script_args.input_media_path, script_args.input_text_path, script_args.output_json_path)
    elif script_args.original_json_path and script_args.updated_json_path:
        assert not script_args.output_json_path.exists(), f"File {script_args.output_json_path} exists!"
        update_alignments(script_args.input_media_path,
                          script_args.original_json_path,
                          script_args.updated_json_path,
                          script_args.output_json_path)
